...
impl<T> Tx<T> {
    pub fn send(&mut self, msg: T) -> Result<(), crate::SendError<T>> {
        self.0
            .send(msg)
            .map_err(|flume::SendError(msg)| crate::SendError::Disconnected(msg))
    }
    pub fn try_send(&mut self, msg: T) -> Result<(), crate::TrySendError<T>> {
        self.0.try_send(msg).map_err(|err| match err {
            flume::TrySendError::Disconnected(msg) => crate::TrySendError::Disconnected(msg),
            flume::TrySendError::Full(msg) => crate::TrySendError::Full(msg),
        })
    }
}
...
impl<T> Rx<T> {
    pub fn recv(&mut self) -> Result<T, crate::RecvError> {
        self.0.recv().map_err(|err| match err {
            flume::RecvError::Disconnected => crate::RecvError::Disconnected,
        })
    }
    pub fn try_recv(&mut self) -> Result<T, crate::TryRecvError> {
        self.0.try_recv().map_err(|err| match err {
            flume::TryRecvError::Empty => crate::TryRecvError::Empty,
            flume::TryRecvError::Disconnected => crate::TryRecvError::Disconnected,
        })
    }
}
